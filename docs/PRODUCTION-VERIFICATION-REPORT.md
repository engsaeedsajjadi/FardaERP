...
5. **Gate 5 GitHub CI** · sandbox (App token, no `workflows` scope) · `git push` with `.github/workflows/ci.yml` · green run on arena branch · `! [remote rejected] … refusing to allow a GitHub App to create or update workflow … without workflows permission` · **BLOCKED-ENV** · 2026-09-16T20:45:33Z. Local: YAML parse + stage order + no-secrets PASS.
