...
| VAT policy engine (settings-driven rate, effective_from, exempt party, GL) | **PARTIAL** | `farda_iran/tax/service.py`, `farda_iran/doctype/farda_vat_settings/` | covered live (R3: 10% net→GL credit, exempt customer→0) | R3 | none | settings singleton UI | Farda VAT Settings singleton | account-company check on validate | before_migrate seeds defaults | Engine core ready | **item-level taxable/non-taxable MISSING (verified: no item logic in service.py)**; VAT report MISSING; custom-rate/0%/effective-date runtime matrix; tax templates; rounding case |
...
1. **VAT completion** (item-level taxable/non-taxable + VAT report + rate-matrix runtime) — *selected: highest-priority PARTIAL with unfinished business logic; order items 3–4 (Jalali/Toman cores) are verified, remainder tracked in rows 1–2/11*
