...
| Jalali central service | **MISSING** | CRITICAL | nothing implemented; DB stays Gregorian (policy) |
| IRR/Toman central service | **MISSING** | CRITICAL | IRR currency seeding works (G5), no Toman display layer |
...
| Persian text normalization + search | **MISSING** | HIGH | ي/ی، ك/ک، digits — no fold service |
...
| Payment gateway abstraction (ZarinPal/IDPay/NextPay) | **MISSING** | HIGH | adapter architecture absent; secrets policy defined (.env only) |
| SMS provider abstraction + OTP | **MISSING** | HIGH | Frappe SMS gateway is generic; no Iranian providers/OTP service |
| Persian invoice print / PDF | **MISSING** | HIGH | default English print formats; Persian PDF font pipeline untested |
| RTL / Persian UX | **PARTIAL** | HIGH | fa translation file exists upstream but ~2,488 empty msgids (needs audit/fix); no RTL-specific CSS work |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| C1 | Jalali central service | MISSING | all | `farda_iran/jalali/` pure-Python service (to/from/format/parse/leap/fiscal) + doc-type hooks for display | `erpnext/farda_iran/jalali/*` | none (DB Gregorian) | REST `farda.jalali.*` | date pickers/format v2 | roundtrip+leap+boundary unit suite | none | none | 0% |
| C2 | IRR/Toman monetary service | MISSING | all | `farda_iran/currency/` single-source ratio + conversion + rounding + formatting | `erpnext/farda_iran/currency/*` | none | REST `farda.currency.*` | Toman display layer | financial integrity suite (1T=10R, rounding, neg, large) | none | none | 0% |
...
| H2 | Persian normalization/search | MISSING | fold service, digit fold, ZWNJ | `farda_iran/utilities/normalization.py` + search hooks | utilities | none | fold API | live-search behavior | normalization suite | none | none | 0% |
...
| H7 | Persian invoice/PDF/print | MISSING | RTL print formats A4/thermal, Persian PDF font pipeline | `farda_iran/printing/` formats + font assets + PDF tests | printing | none | PDF endpoint (existing) | print formats | PDF rendering test (glyph/RTL checks) | none | none | 0% |
| H8 | RTL/Persian UX | PARTIAL | full fa coverage (fa.po has ~2,488 empty msgids), RTL css, digits | translation audit+fill, RTL stylesheets, brand strings | `farda_iran/` + translation files | none | none | everything | screenshot/manual + msgid audit script | none | none | 15% (upstream fa base) |
